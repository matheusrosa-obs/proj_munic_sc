...
# ------------------------------------
# EXECUTA REGRESSÕES E ARMAZENA OS COEFICIENTES
# ------------------------------------
regression_results = {}

for munic, group in df_munic.dropna(subset=['pib_munic', 'pib_mesoregiao']).groupby('Munic'):
    model = sm.ols('pib_munic ~ pib_mesoregiao', data=group).fit(cov_type='HC3')
...
# ------------------------------------
# CARREGAMENTO DAS PROJEÇÕES DAS MESORREGIÕES
# ------------------------------------
...
# ------------------------------------
# EMPILHANDO AS PREVISÕES E SALVANDO EM EXCEL
# ------------------------------------
